...
#include "factor/baro_factor.hpp"
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const double header)
{
    ...
    if (solver_flag == INITIAL)
    {
        ...
        if(frame_count < WINDOW_SIZE)
        {
            ...
            baro_measure[frame_count] = baro_measure[prev_frame];
			Bbs[frame_count]		  = Bbs[prev_frame];
        }
...
void Estimator::optimization()
{
    ...
    for (int i = 0; i < frame_count + 1; i++)
    {
        ...
        if (USE_BARO)
			problem.AddParameterBlock(para_baro[i], SIZE_BARO);
    }
    if(!USE_IMU)
    ...
    if (USE_BARO) {
		const double baro_measure_sqrt_info = sqrt(1.0 / BARO_MEASURE_NOISE);
		const double baro_bias_sqrt_info	= sqrt(1.0 / BARO_BIAS_NOISE);
		for (int i = 0; i < frame_count; i++) {
			int	 j			 = i + 1;
			auto baro_factor = BaroFactor::Create(baro_measure[i], baro_measure_sqrt_info);
			problem.AddResidualBlock(baro_factor, NULL, para_Pose[i], para_baro[i]);
			auto baro_bias_factor = BaroBiasFactor::Create(baro_bias_sqrt_info);
			problem.AddResidualBlock(baro_bias_factor, NULL, para_baro[i], para_baro[j]);
		}
	}
...
void Estimator::slideWindow()
{
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        if (frame_count == WINDOW_SIZE)
        {
            for (int i = 0; i < WINDOW_SIZE; i++)
            {
                ...
                if (USE_BARO) {
					swap(baro_measure[i], baro_measure[i + 1]);
					swap(Bbs[i], Bbs[i + 1]);
				}
            }
            ...
            if (USE_BARO) {
				baro_measure[WINDOW_SIZE] = baro_measure[WINDOW_SIZE - 1];
				Bbs[WINDOW_SIZE]		  = Bbs[WINDOW_SIZE - 1];
			}
    ...
    else
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            if (USE_BARO) {
				baro_measure[frame_count - 1] = baro_measure[frame_count];
				Bbs[frame_count - 1]		  = Bbs[frame_count];
			}
            slideWindowNew();
        }
    }
