...
void ground_truth_callback(const gazebo_msgs::ModelStatesPtr &msg) {
	...
	foutC.precision(5);
	foutC << pose.position.x << " " << pose.position.y << " " << pose.position.z << " " << pose.orientation.x << " "
		  << pose.orientation.y << " " << pose.orientation.z << " " << pose.orientation.w << endl;
	foutC.close();
...
#ifndef WITH_CUDA
	if (USE_GPU) {
		ROS_ERROR("you should compile the code with ENABLE_GPU=true (in CmakeLists.txt) for USE_GPU=1 (defined at %s)",
				  config_file.c_str());
		exit(-1);
	}
#endif
