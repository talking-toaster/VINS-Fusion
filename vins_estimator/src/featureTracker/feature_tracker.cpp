...
FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1) {
	static double track_image_sum		= 0;
	static double detect_time_sum		= 0;
	static double flow_track_time_sum	= 0;
	static double copy_gpu_img_time_sum = 0;
	static int	  count					= 0;
	count++;
	double track_image_time = 0, flow_track_left = 0, flow_track_right = 0, detect_time = 0, copy_gpu_img_time = 0;
	TicToc t_track_image;
	cur_time		 = _cur_time;
	cur_img			 = _img;
	row				 = cur_img.rows;
	col				 = cur_img.cols;
...
#endif
	copy_gpu_img_time = t_copy.toc();
	copy_gpu_img_time_sum += copy_gpu_img_time;
	TicToc t_flow_left;
	if (prev_pts.size() > 0) {
		vector<uchar> status;
		vector<float> err;
		if (!USE_GPU) {
			...
			if (FLOW_BACK) {
				...
				cv::calcOpticalFlowPyrLK(cur_img, prev_img, cur_pts, reverse_pts, reverse_status, err, cv::Size(21, 21),
										 ...
										 cv::OPTFLOW_USE_INITIAL_FLOW);
				// cv::calcOpticalFlowPyrLK(cur_img, prev_img, cur_pts, reverse_pts, reverse_status, err, cv::Size(21,
				// 21), 3);
				for (size_t i = 0; i < status.size(); i++) {
	...
	flow_track_left = t_flow_left.toc();
	flow_track_time_sum += flow_track_left;
	...
	flow_track_right = t_flow_right.toc();
	flow_track_time_sum += flow_track_right;
	...
	track_image_time = t_track_image.toc();
	track_image_sum += track_image_time;
	if (count == 1) {
		track_image_sum		  = 0;
		flow_track_time_sum	  = 0;
		detect_time_sum		  = 0;
		copy_gpu_img_time_sum = 0;
	}
	ROS_INFO("TrackImage: track_left: %.3fms, track_right: %.3fms , avg: %.3fms, detect: %.3fms avg: %.3fms, "
			 "copy_gpu_img: %.3fms avg: %.3fms , all: %.3fms avg: %.3fms",
			 flow_track_left, flow_track_right, flow_track_time_sum / count, detect_time, detect_time_sum / count,
			 copy_gpu_img_time, copy_gpu_img_time_sum / count, track_image_time, track_image_sum / count);
	return featureFrame;
}
...
void FeatureTracker::drawTrack(const cv::Mat &imLeft, const cv::Mat &imRight, vector<int> &curLeftIds,
	...
	for (size_t j = 0; j < curLeftPts.size(); j++) {
		...
		cv::circle(imTrack, curLeftPts[j], 2, cv::Scalar(0, 255 * len, 255 * (1 - len)),
				   2); // BGR  , 追踪次数递增, 红色->绿色
	}
	if (!imRight.empty() && stereo_cam) {
